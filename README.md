...
## 6. 系統限制與未來改進
1. **檢索多維度不足**：目前僅依賴相似度檢索（Vector Search）。未來計畫引入 Hybrid Search（結合 BM25 關鍵字檢索），這能顯著提升對於特定學術術語、模型名稱或論文標題的搜尋命中率。

2. **推論效能優化**：由於 gpt-oss:20b 模型較大，在伺服器負載較高時回應較慢。未來計畫實作 Streaming Output（串流輸出），透過即時回傳 Token 來改善使用者在等待回答時的體感延遲，優化互動體驗。

3. **解析深度與多模態處理**：目前使用 pypdf 進行純文字提取，對於論文中的複雜圖表、數學公式解析度較低，容易導致資訊遺失。未來嘗試導入視覺化解析模型（如 LayoutLM 或 Nougat）以獲取更精確的結構化內容。